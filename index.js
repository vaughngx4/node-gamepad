...
Gamepad.prototype._loadConfiguration = function () {
  var configPath = path.resolve(
    __dirname,
    "./controllers/" + this._type + ".json"
  );
  if (!fs.existsSync(configPath)) {
    console.log(
      ('The controller configuration for "' + this._type + '" does not exist.')
        .red
    );
    process.exit(0);
  }
...
Gamepad.prototype._detectControllerConfiguration = function () {
  ...
  var platformPath = path.resolve(
    __dirname,
    "./controllers/" + this._type + "/"
  );
  ...
  for (var i = 0, len = files.length; i < len; i++) {
    ...
    for (var j = 0, leng = devices.length; j < leng; j++) {
      tmpDevice = devices[j];
      if (
        tmpConfig.vendorID === tmpDevice.vendorId &&
        tmpConfig.productID === tmpDevice.productId
      ) {
...
Gamepad.prototype.connect = function () {
  if (!this._detectControllerConfiguration()) {
    console.log(
      ('A product for the vendor "' + this._type + '" could not be detected.')
        .red
    );
    process.exit(0);
  }
  ...
  if (
    typeof this._options.debug === "boolean" &&
...
Gamepad.prototype._processJoysticks = function (data) {
  ...
  var joysticks = this._config.joysticks,
    joystick,
    currentState;
...
Gamepad.prototype._processStatus = function (data) {
  ...
  var statuses = this._config.status,
    status,
    state,
    states;
  var currentState;
